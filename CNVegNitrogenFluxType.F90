module CNVegNitrogenFluxType
  ...
  type, public :: cnveg_nitrogenflux_type
     ...
     ! herbivory flux
     real(r8), pointer :: leafn_to_herbiv_patch                     (:)     ! leaf N to herbivore (gC/m2/s)   
     real(r8), pointer :: leafn_to_herbiv_col                       (:)     ! (gN/m2/s) column herbivory loss (p2c)
     real(r8), pointer :: herbivn_to_litter_patch                   (:)     ! herbivore N to litter (gC/m2/s)   
     real(r8), pointer :: herbivn_to_litter_col                     (:)     ! (gN/m2/s) column herbivory frass (p2c)
     real(r8), pointer :: herbivn_xfer_patch                        (:)     ! N to biomass pool (gC/m2/s)
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(this%leafn_to_herbiv_patch                     (begp:endp)) ; this%leafn_to_herbiv_patch                     (:) = nan
    allocate(this%leafn_to_herbiv_col                       (begc:endc)) ; this%leafn_to_herbiv_col                       (:) = nan
    allocate(this%herbivn_to_litter_patch                   (begp:endp)) ; this%herbivn_to_litter_patch                      (:) = nan
    allocate(this%herbivn_to_litter_col                     (begc:endc)) ; this%herbivn_to_litter_col                        (:) = nan
    allocate(this%herbivn_xfer_patch                        (begp:endp)) ; this%herbivn_xfer_patch                        (:) = nan
  ...
  subroutine InitHistory(this, bounds)
    ...
    ! shsun
    this%leafn_to_herbiv_patch(begp:endp) = spval
    call hist_addfld1d (fname='LEAFN_TO_HERBIV', units='gN/m^2/s', &
         avgflag='A', long_name='leaf N to herbivore', &
         ptr_patch=this%leafn_to_herbiv_patch)
    this%herbivn_to_litter_patch(begp:endp) = spval
    call hist_addfld1d (fname='HERBIVN_TO_LITTER', units='gN/m^2/s', &
         avgflag='A', long_name='herbivore N to litter', &
         ptr_patch=this%herbivn_to_litter_patch)
    this%herbivn_xfer_patch(begp:endp) = spval
    call hist_addfld1d (fname='HERBIVN_XFER', units='gN/m^2/s', &
         avgflag='A', long_name='herbivore biomass N flux', &
         ptr_patch=this%herbivn_xfer_patch)
...
! End BACKWARDS_COMPATIBILITY(wrw, 2018-06-28) re. issue #426
  ...
  subroutine SetValues ( this, &
    ...
    do fi = 1,num_patch
       ...
       this%leafn_to_herbiv_patch(i)                     = value_patch
       this%herbivn_to_litter_patch(i)                   = value_patch
    ...
    do fi = 1,num_column
       ...
       ! herbivory
       this%leafn_to_herbiv_col(i)                  = value_column
       this%herbivn_to_litter_col(i)                = value_column
