...
contains
  ...
  subroutine CBalanceCheck(this, bounds, num_soilc, filter_soilc, &
    ...
    associate(                                                                            & 
         ...
         herbivc_to_litter       =>    cnveg_carbonflux_inst%herbivc_to_litter_col      , & 
         col_insect_closs        =>    cnveg_carbonflux_inst%leafc_to_herbiv_col        , & ! (gC/m2/s) column herbivory loss
         ar => cnveg_carbonflux_inst%ar_col      , & ! test
      ...
      do fc = 1,num_soilc
         ...
         ! shsun test: take account of insect loss, frass carbon may be double
         ! counted during litterfall   
         col_coutputs = col_coutputs + col_insect_closs(c) 
         ...
         ! shsun test 9/28/2021
         if (abs(col_errcb(c)) > 1._r8) then
            err_found = .true.
            err_index = c
         end if
          if (abs(col_errcb(c)) > 1.e-2_r8) then
            write(iulog,*) 'cbalance warning',c,col_errcb(c),col_endcb(c)
            write(iulog,*) 'insect_closs, herbivc_to_litter', col_insect_closs(c), herbivc_to_litter(c)
         end if
  ...
  subroutine NBalanceCheck(this, bounds, num_soilc, filter_soilc, &
    ...
    associate(                                                                             & 
      ...
      do fc = 1,num_soilc
         ...
         ! shsun test 9/28/2021
         if (abs(col_errnb(c)) > 1._r8) then
            err_found = .true.
            err_index = c
         end if
         if (abs(col_errnb(c)) > 1e-3_r8) then
            write(iulog,*) 'nbalance warning',c,col_errnb(c),col_endnb(c)
            write(iulog,*)'som_n_leached',som_n_leached(c)
            write(iulog,*)'sminn_leached',sminn_leached(c)
            write(iulog,*)'Latdeg,Londeg =',grc%latdeg(col%gridcell(c)),grc%londeg(col%gridcell(c))
