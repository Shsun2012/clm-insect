    ...
    associate(                                                               & 
      ...
      if (.not. use_fates) then    
        do fp = 1,num_soilp
           ...
           ! shsun: test herbivc pool
           cs_veg%herbivc_patch(p) = cs_veg%herbivc_patch(p) + cf_veg%herbivc_xfer_patch(p)*dt
           cs_veg%leafc_patch(p) = cs_veg%leafc_patch(p) - cf_veg%leafc_to_herbiv_patch(p)*dt
