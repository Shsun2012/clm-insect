  ...
  subroutine NStateUpdate1(num_soilc, filter_soilc, num_soilp, filter_soilp, &
    ...
    associate(                                                                   & 
      ...
      do fp = 1,num_soilp
         ...
         ! herbivory (shsun) 
         ns_veg%leafn_patch(p) = ns_veg%leafn_patch(p) - nf_veg%leafn_to_herbiv_patch(p)*dt
         ns_veg%herbivn_patch(p)  = ns_veg%herbivn_patch(p)  + nf_veg%leafn_to_herbiv_patch(p)*dt 
