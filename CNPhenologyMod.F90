...
contains
  ...
  subroutine CNPhenology (bounds, num_soilc, filter_soilc, num_soilp, &
    ...
    if ( phase == 1 ) then
       ...
       ! For herbivory effects (shsun Mar 2021)
       call CNHerbivory(num_soilp, filter_soilp,   &
            temperature_inst, cnveg_state_inst, cnveg_carbonstate_inst, cnveg_nitrogenstate_inst, cnveg_carbonflux_inst, cnveg_nitrogenflux_inst, & 
            leaf_prof_patch, bounds, num_soilc, filter_soilc)
  ...
  !-----------------------------------------------------------------------
  subroutine CNHerbivory (num_soilp, filter_soilp, &
       temperature_inst, cnveg_state_inst, cnveg_carbonstate_inst, cnveg_nitrogenstate_inst, cnveg_carbonflux_inst, cnveg_nitrogenflux_inst, &
       leaf_prof_patch, bounds, num_soilc, filter_soilc)
    !
    ! !DESCRIPTION:
    ! Determines the flux of C and N from displayed pools to herbivory pools
    ! divided by frass, respiration, and biomass (shsun Mar 2021).
    !
    ! !USES:
    use CNSharedParamsMod   , only : use_fun
    use clm_varctl          , only : CNratio_floating, iulog, CN_evergreen_phenology_opt    
    use clm_varcon          , only : secspday
    use clm_time_manager    , only : get_days_per_year
    ...
    integer                       , intent(in)    :: num_soilc       ! number of soil columns in filter
    integer                       , intent(in)    :: filter_soilp(:) ! filter for soil patches
    integer                       , intent(in)    :: filter_soilc(:) ! filter for soil patches
    type(cnveg_state_type)        , intent(inout) :: cnveg_state_inst
    ...
    real(r8):: topt                       ! optimal temperature for herbivory
    real(r8):: sigmap                     ! half-width of the Gaussian decline in growth rate at temperature below topt
    real(r8):: ctmax                      ! maximum critical temperature for herbivory
    real(r8):: lat,lon
    real(r8):: tempc
    real(r8):: fr_herbiv
    !-----------------------------------------------------------------------
    ...
    associate(                                                                     & 
         ...
         t_ref2m           =>    temperature_inst%t_ref2m_patch                  , & ! Input: [real(r8) (:) ]  daily minimum of average 2 m height surface air temperature (K)
         ...
         leafc_to_herbiv   =>    cnveg_carbonflux_inst%leafc_to_herbiv_patch     , & ! Output: [real(r8) (:) ] biomass C flux from leaves to herbivore
         herbivc_xfer      =>    cnveg_carbonflux_inst%herbivc_xfer_patch        , & ! Output: [real(r8) (:) ] C flux to herbivore pool
         herbivc_to_litter =>    cnveg_carbonflux_inst%herbivc_to_litter_patch   , & ! Output: [real(r8) (:) ] frass C flux
         leafn_to_herbiv   =>    cnveg_nitrogenflux_inst%leafn_to_herbiv_patch   , & ! Output: [real(r8) (:)] biomass N flux from leaves to herbivore
         herbivn_xfer      =>    cnveg_nitrogenflux_inst%herbivn_xfer_patch      , & ! Output: [real(r8) (:) ] N flux to herbivore pool
         herbivn_to_litter =>    cnveg_nitrogenflux_inst%herbivn_to_litter_patch , & ! Output: [real(r8) (:) ] frass N flux
         herbivc_to_resp   =>    cnveg_carbonflux_inst%herbivc_to_resp_patch     , & ! Output: [real(r8) (:) ] herbivore respiration C flux
      ...
      do fp = 1,num_soilp
         ...
         c = patch%column(p)
         g = patch%gridcell(p)
         lat = abs(grc%latdeg(g))
         lon = grc%londeg(g)

         if (c /= 6531._r8 .and. c /= 59213._r8) then 
            topt = -0.13_r8*lat + 31.79_r8
            sigmap = 0.166_r8*lat + 0.75_r8
            ctmax = -0.12_r8*lat + 36.53_r8
            tempc = t_ref2m(p) - 273.5_r8 

            if (tempc < topt) then
              fr_herbiv = 0.05_r8*exp(-((tempc - topt)/(2._r8*sigmap))**2._r8)*exp(0.6_r8/8.617e5_r8*(1._r8/topt - 1._r8/tempc))/(leaf_long(ivt(p))*dayspyr*secspday)
            else
              fr_herbiv = 0.05_r8*(1._r8 - ((tempc - topt)**2._r8/(topt - ctmax)**2._r8))*exp(0.6_r8/8.617e5_r8*(1._r8/topt - 1._r8/tempc))/(leaf_long(ivt(p))*dayspyr*secspday)
            end if

            if (fr_herbiv < 0._r8) then
              fr_herbiv = 0._r8
            end if

            if (t_ref2m(p) < 1.e30_r8 .and. tempc > 0._r8 .and. fr_herbiv < 1._r8 .and. leafc(p) > 0._r8) then
               ! units for fr_herbiv shouldbe 1/s ?
               leafc_to_herbiv(p) = leafc(p)*fr_herbiv
               leafn_to_herbiv(p) = leafc_to_herbiv(p)/leafcn(ivt(p))            

               if (CN_evergreen_phenology_opt == 1) then
                  herbivc_to_litter(p) = leafc_to_herbiv(p)*0.7_r8
                  herbivc_to_resp(p) = leafc_to_herbiv(p)*0.2_r8
                  herbivc_xfer(p) = leafc_to_herbiv(p)*0.1_r8
               else
                  herbivc_to_litter(p) = leafc_to_herbiv(p)*0.5_r8
                  herbivc_to_resp(p) = leafc_to_herbiv(p)*0.33_r8
                  herbivc_xfer(p) = leafc_to_herbiv(p)*0.17_r8
               end if

            else
               leafc_to_herbiv(p) = 0._r8
               leafn_to_herbiv(p) = 0._r8
               herbivc_to_litter(p) = 0._r8
               herbivc_to_resp(p) = 0._r8
               herbivc_xfer(p) = 0._r8
            end if 
            herbivn_to_litter(p) = herbivc_to_litter(p)/leafcn(ivt(p))
            herbivn_xfer(p) = herbivc_xfer(p)/leafcn(ivt(p))
          end if
      end do         

      ! frass decomposition calculated here
      do j = 1, nlevdecomp
         do pi = 1,max_patch_per_col
            do fc = 1,num_soilc
               ...
               if ( pi <=  col%npatches(c) ) then
                  p = col%patchi(c) + pi - 1
                  if (patch%active(p) .and. leaf_prof(p,j) > 0._r8 ) then
                     ! leaf litter carbon fluxes
                     phenology_c_to_litr_met_c(c,j) = phenology_c_to_litr_met_c(c,j) &
                          + herbivc_to_litter(p) * lf_flab(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                     phenology_c_to_litr_cel_c(c,j) = phenology_c_to_litr_cel_c(c,j) &
                          + herbivc_to_litter(p) * lf_fcel(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                     phenology_c_to_litr_lig_c(c,j) = phenology_c_to_litr_lig_c(c,j) &
                          + herbivc_to_litter(p) * lf_flig(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                     ! leaf litter nitrogen fluxes
                     phenology_n_to_litr_met_n(c,j) = phenology_n_to_litr_met_n(c,j) &
                          + herbivn_to_litter(p) * lf_flab(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                     phenology_n_to_litr_cel_n(c,j) = phenology_n_to_litr_cel_n(c,j) &
                          + herbivn_to_litter(p) * lf_fcel(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                     phenology_n_to_litr_lig_n(c,j) = phenology_n_to_litr_lig_n(c,j) &
                          + herbivn_to_litter(p) * lf_flig(ivt(p)) * wtcol(p) * leaf_prof(p,j)
                  end if
              end if
            end do
         end do
  ...
  subroutine CNOffsetLitterfall (num_soilp, filter_soilp, &
    ...
    associate(                                                                           & 
         ...
         herbivc_to_litter     =>    cnveg_carbonflux_inst%herbivc_to_litter_patch     , & ! Input: [real(r8) (:) ] frass C flux 
  ...
  subroutine CNBackgroundLitterfall (num_soilp, filter_soilp, &
    ...
    associate(                                                                     & 
         ...
         herbivc_to_litter =>    cnveg_carbonflux_inst%herbivc_to_litter_patch   , &
  ...
  subroutine CNLitterToColumn (bounds, num_soilc, filter_soilc,         &
    ...
    use clm_varpar , only : max_patch_per_col, nlevdecomp
    use pftconMod  , only : npcropmin
    use clm_varctl , only : use_grainproduct, iulog 
    !
    ! !ARGUMENTS:
    type(bounds_type)               , intent(in)    :: bounds
    integer                         , intent(in)    :: num_soilc       ! number of soil columns in filter
    ...
    associate(                                                                                & 
         ...
         herbivc_to_litter         => cnveg_carbonflux_inst%herbivc_to_litter_patch          , & ! Input:  [real(r8) (:)   ]  herbivore C to litter (gC/m2/s)
            ...
            do fc = 1,num_soilc
               ...
               if ( pi <=  col%npatches(c) ) then
                  ...
                  if (patch%active(p)) then
                     ...
                     if (leafc_to_litter(p) < 0._r8) then
                       write (iulog,*) 'leafc_to_litter < 0'
                     end if 
                     ! leaf litter carbon fluxes
                     phenology_c_to_litr_met_c(c,j) = phenology_c_to_litr_met_c(c,j) &
