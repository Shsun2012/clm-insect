...
#include "shr_assert.h"
  ...
  type, public :: cnveg_carbonflux_type
     ...
     ! herbivory flux (Added by shsun Mar 2021)
     real(r8), pointer :: leafc_to_herbiv_patch                     (:)     ! leaf C to herbivore (gC/m2/s)   
     real(r8), pointer :: leafc_to_herbiv_col                       (:)     ! (gC/m2/s) column herbivory loss (p2c)
     real(r8), pointer :: herbivc_to_litter_patch                   (:)     ! herbivore C to litter (gC/m2/s)   
     real(r8), pointer :: herbivc_to_litter_col                     (:)     ! (gC/m2/s) column herbivore frass (p2c)
     real(r8), pointer :: herbivc_to_resp_patch                     (:)     ! herbivore C to respiration (gC/m2/s)
     real(r8), pointer :: herbivc_to_resp_col                       (:)     ! (gC/m2/s) column herbivore respiration (p2c)
     real(r8), pointer :: herbivc_xfer_patch                        (:)     ! herbivore C biomass (gC/m2/s)
...
contains
  ...
  subroutine InitAllocate(this, bounds, carbon_type)
    ...
    allocate(this%leafc_to_herbiv_patch                     (begp:endp)) ; this%leafc_to_herbiv_patch                     (:) = nan
    allocate(this%leafc_to_herbiv_col                       (begc:endc)) ; this%leafc_to_herbiv_col                       (:) = nan
    allocate(this%herbivc_to_litter_patch                   (begp:endp)) ; this%herbivc_to_litter_patch                   (:) = nan
    allocate(this%herbivc_to_litter_col                     (begc:endc)) ; this%herbivc_to_litter_col                     (:) = nan
    allocate(this%herbivc_to_resp_patch                     (begp:endp)) ; this%herbivc_to_resp_patch                     (:) = nan
    allocate(this%herbivc_to_resp_col                       (begc:endc)) ; this%herbivc_to_resp_col                       (:) = nan
    allocate(this%herbivc_xfer_patch                        (begp:endp)) ; this%herbivc_xfer_patch                        (:) = nan
  ...
  subroutine InitHistory(this, bounds, carbon_type)
    ...
    if (carbon_type == 'c12') then
       ...
       this%leafc_to_herbiv_patch(begp:endp) = spval
       call hist_addfld1d (fname='LEAFC_TO_HERBIV', units='gC/m^2/s', &
            avgflag='A', long_name='leaf C herbivory', &
            ptr_patch=this%leafc_to_herbiv_patch, default='inactive')

    ! shsun test
    !   this%herbivc_to_litter_patch(begp:endp) = spval
    !   call hist_addfld1d (fname='HERBIVC_TO_LITTER', units='gC/m^2/s', &
    !        avgflag='A', long_name='herbivore C to litter', &
    !        ptr_patch=this%herbivc_to_litter_patch, default='inactive')

    !   this%herbivc_to_resp_patch(begp:endp) = spval
    !   call hist_addfld1d (fname='HERBIVC_TO_RESP', units='gC/m^2/s', &
    !        avgflag='A', long_name='herbivore C to respiration', &
    !        ptr_patch=this%herbivc_to_resp_patch, default='inactive')

       this%herbivc_xfer_patch(begp:endp) = spval
       call hist_addfld1d (fname='HERBIVC_XFER', units='gC/m^2/s', &
            avgflag='A', long_name='herbivore C flux to biomass', &
            ptr_patch=this%herbivc_xfer_patch, default='inactive')
...
! FUN Ends 
    ...
    if ( carbon_type == 'c13') then
       ...
       this%leafc_to_herbiv_patch(begp:endp) = spval
       call hist_addfld1d (fname='C13_LEAFC_TO_HERBIV', units='gC13/m^2/s', &
            avgflag='A', long_name='C13 leaf C to herbivore', &
            ptr_patch=this%leafc_to_herbiv_patch, default='inactive')

       this%herbivc_to_litter_patch(begp:endp) = spval
       call hist_addfld1d (fname='C13_HERBIVC_TO_LITTER', units='gC13/m^2/s', &
            avgflag='A', long_name='C13 herbivore C to litter', &
            ptr_patch=this%herbivc_to_litter_patch, default='inactive')

       this%herbivc_to_resp_patch(begp:endp) = spval
       call hist_addfld1d (fname='C13_HERBIVC_TO_RESP', units='gC13/m^2/s', &
            avgflag='A', long_name='C13 herbivore C to respiration', &
            ptr_patch=this%herbivc_to_resp_patch, default='inactive')
    ...
    if ( carbon_type == 'c14' ) then
       ...
       this%leafc_to_herbiv_patch(begp:endp) = spval
       call hist_addfld1d (fname='C14_LEAFC_TO_HERBIV', units='gC14/m^2/s', &
            avgflag='A', long_name='C14 leaf C herbivore', &
            ptr_patch=this%leafc_to_herbiv_patch, default='inactive')

       this%herbivc_to_litter_patch(begp:endp) = spval
       call hist_addfld1d (fname='C14_HERBIVC_TO_LITTER', units='gC14/m^2/s', &
            avgflag='A', long_name='C14 herbivore C to litter', &
            ptr_patch=this%herbivc_to_litter_patch, default='inactive')

       this%herbivc_to_resp_patch(begp:endp) = spval
       call hist_addfld1d (fname='C14_HERBIVC_TO_RESP', units='gC14/m^2/s', &
            avgflag='A', long_name='C14 herbivore C to litter', &
            ptr_patch=this%herbivc_to_resp_patch, default='inactive')
    ...
    if (carbon_type == 'c12') then
       ...
       ! shsun test 
       this%herbivc_to_litter_col(begc:endc) = spval
       call hist_addfld1d (fname='HERBIVC_TO_LITTER', units='gC/m^2/s', &
            avgflag='A', long_name='insect herbivory C to litter', &
            ptr_col=this%herbivc_to_litter_col)

       this%herbivc_to_resp_col(begc:endc) = spval
       call hist_addfld1d (fname='HERBIVC_TO_RESP', units='gC/m^2/s', &
            avgflag='A', long_name='insect herbivory C to respiration', &
            ptr_col=this%herbivc_to_resp_col)
  ...
  subroutine SetValues ( this, &
    ...
    do fi = 1,num_patch
       ...
       ! shsun
       this%leafc_to_herbiv_patch(i)                     = value_patch
       this%herbivc_to_litter_patch(i)                   = value_patch
       this%herbivc_to_resp_patch(i)                     = value_patch
       this%herbivc_xfer_patch(i)                        = value_patch
    ...
    do fi = 1,num_column
       ...
       ! shsun
       this%leafc_to_herbiv_col(i)     = value_column
       this%herbivc_to_litter_col(i)   = value_column
       this%herbivc_to_resp_col(i)     = value_column
  ...
  subroutine Summary_carbonflux(this, &
    ...
    ! shsun
    call p2c(bounds, num_soilc, filter_soilc, &
         this%leafc_to_herbiv_patch(bounds%begp:bounds%endp), &
         this%leafc_to_herbiv_col(bounds%begc:bounds%endc))

    call p2c(bounds, num_soilc, filter_soilc, &
         this%herbivc_to_litter_patch(bounds%begp:bounds%endp), &
         this%herbivc_to_litter_col(bounds%begc:bounds%endc))

    call p2c(bounds, num_soilc, filter_soilc, &
         this%herbivc_to_resp_patch(bounds%begp:bounds%endp), &
         this%herbivc_to_resp_col(bounds%begc:bounds%endc))
